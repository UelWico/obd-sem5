...
const table_name = "Табель учета рабочего времени";
const create_button_text = "Добавить запись";
const create_title_text = "Добавление записи";
const update_button_text = "Cохранить";
const update_title_text = "Изменение записи";
const attribute_name_item_id = "timesheet_id";
...
const headers = [
  ...
  {
    header_name: "Присутствие сотрудника",
    header_width: "150px",
  },
  {
    header_name: "Причина отсутствия",
    header_width: "150px",
  },
  {
    header_name: "Дата записи",
    header_width: "150px",
  },
];
...
const get_items_func = requests.POST_get_timesheets;
const get_item_func = requests.POST_get_timesheet;
const create_item_func = requests.POST_create_timesheet;
const update_item_func = requests.PUT_update_timesheet;
const delete_item_func = requests.POST_delete_timesheet;

const new_create_form = function () {
  return {
    new_item: objects.NewCreateTimesheet({
      staff_id: 1,
      timesheet_presence: true,
      absence_type_id: 1,
      timesheet_date: "",
    }),
    is_create: true,
...
export default function Timesheet() {
  ...
  const [absence_types, set_absence_types] = useState([]);
  useEffect(() => {
    requests.POST_get_absence_types().then((absence_type_list) => {
      set_absence_types(absence_type_list);
    });
  }, []);
  ...
  const get_edit_func = function (obj) {
    return function () {
      get_item_func(obj).then((out) => {
        set_form({
          new_item: objects.NewUpdateTimesheet(out),
          is_create: false,
        });
      });
    };
        ...
        >
          ...
          >
            ...
            <input
              ...
              type="checkbox"
              value=""
              checked={form.new_item.timesheet_presence}
              onChange={(e) => {
                set_form_attribute("timesheet_presence", e.target.checked);
              }}
            />
          </div>
          <p style={{ marginBottom: 6 }}>Причина отсутствия</p>
          <select
            style={{
              fontFamily: "system-ui",
              fontSize: 16,
            ...
            placeholder="AbsenceType"
            value={form.new_item.absence_type_id}
            onChange={(e) =>
              set_form_attribute("absence_type_id", e.target.value)
            }
          >
            {absence_types
              ? absence_types.map((db) => (
                  <option
                    key={db.absence_type_id}
                    value={db.absence_type_id}
                    label={db.absence_type_name}
                  ></option>
                ))
              : "Загрузка..."}
          </select>
                ...
                ? items
                    ...
                    .map((item) => (
                      <tr key={item[attribute_name_item_id]}>
                        ...
                        <td>
                          {item.timesheet_presence
                            ? "Присутствует"
                            : "Отсутствует"}
                        </td>
                        <td>{item.absence_type.absence_type_name}</td>
                        <td>
                          {new Date(
                            item.timesheet_date.getTime() -
                        ...
                        <td>
                          <ActionButtons
                            delFunc={get_del_func({
                              timesheet_id: item[attribute_name_item_id], ////
                            })}
