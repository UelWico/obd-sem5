...
const table_name = "Поставки";
const create_button_text = "Добавить поставку";
const create_title_text = "Добавление поставки";
const update_button_text = "Cохранить";
const update_title_text = "Изменение поставки";
const attribute_name_item_id = "delivery_id";

const headers = [
  {
    header_name: "Принято сотрудником",
    header_width: "150px",
  },
  {
    header_name: "Поставщик",
    header_width: "150px",
  },
  {
    header_name: "Тип поставки",
    header_width: "150px",
  },
  {
    header_name: "Дата поставки",
    header_width: "150px",
  },
];
...
const get_items_func = requests.POST_get_deliveries;
const get_item_func = requests.POST_get_delivery;
const create_item_func = requests.POST_create_delivery;
const update_item_func = requests.PUT_update_delivery;
const delete_item_func = requests.POST_delete_delivery;

const new_create_form = function () {
  return {
    new_item: objects.NewCreateDelivery({
      delivery_date: "",
      supplier_id: 1,
      delivery_type_id: 1,
    }),
...
export default function Delivery() {
  ...
  useEffect(() => {
    requests.POST_get_suppliers().then((supplier_list) => {
      set_suppliers(supplier_list);
    });
  }, []);
  const [delivery_types, set_delivery_types] = useState([]);
  useEffect(() => {
    requests.POST_get_delivery_types().then((delivery_type_list) => {
      set_delivery_types(delivery_type_list);
    });
  }, []);
  ...
  const get_edit_func = function (obj) {
    return function () {
      get_item_func(obj).then((out) => {
        set_form({
          new_item: objects.NewUpdateDelivery(out),
          is_create: false,
        });
      });
    };
      ...
      <div className="order-row">
        ...
        >
          ...
          <TableInput
            ...
            label={"Дата концерта"}
          />
          <p style={{ marginBottom: 6 }}>Поставщик</p>
          <select
            style={{
              fontFamily: "system-ui",
              fontSize: 16,
            ...
            value={form.new_item.supplier_id}
            onChange={(e) => set_form_attribute("supplier_id", e.target.value)}
          >
            {suppliers
              ? suppliers.map((db) => (
                  <option
                    key={db.supplier_id}
                    value={db.supplier_id}
                    label={db.supplier_name}
                  ></option>
                ))
              : "Загрузка..."}
          </select>
          <p style={{ marginBottom: 6 }}>Тип поставки</p>
          <select
            style={{
              fontFamily: "system-ui",
              fontSize: 16,
            ...
            value={form.new_item.delivery_type_id}
            onChange={(e) =>
              set_form_attribute("delivery_type_id", e.target.value)
            }
          >
            {delivery_types
              ? delivery_types.map((db) => (
                  <option
                    key={db.delivery_type_id}
                    value={db.delivery_type_id}
                    label={db.delivery_type_name}
                  ></option>
                ))
              : "Загрузка..."}
          </select>
                ...
                ? items
                    ...
                    .map((item) => (
                      <tr key={item[attribute_name_item_id]}>
                        ...
                        <td>{item.delivery_type.delivery_type_name}</td>
                        <td>
                          {new Date(
                            item.delivery_date.getTime() -
                        ...
                        <td>
                          <ActionButtons
                            delFunc={get_del_func({
                              delivery_id: item[attribute_name_item_id], ////
                            })}
